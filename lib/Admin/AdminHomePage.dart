...
import 'package:dept_to_do_app/Admin/DeletedTasks.dart';
import 'package:dept_to_do_app/Admin/StaffList.dart';
import 'package:dept_to_do_app/Admin/TimesUpTasks.dart';
...
// DATA Sorting by Date or Time left

  /*String currentSortOption = 'Date Added'; // Default sorting option

// Function to sort tasks based on selected option
  void sortTasks() {
    if (currentSortOption == 'Date Added') {
      tasks.sort((a, b) {
        final dateA = a["addedDate"] != null
            ? DateTime.parse(a["addedDate"])
            : DateTime.now();
        final dateB = b["addedDate"] != null
            ? DateTime.parse(b["addedDate"])
            : DateTime.now();
        return dateB.compareTo(dateA); // Descending order for date added
      });
      setState(() {});
    } else if (currentSortOption == 'Time Left') {
      tasks.sort((a, b) {
        final timeLeftA = getRemainingTimeValue(a);
        final timeLeftB = getRemainingTimeValue(b);
        print('Task A: ${a["title"]}, Time Left: $timeLeftA');
        print('Task B: ${b["title"]}, Time Left: $timeLeftB');
        return timeLeftA.compareTo(timeLeftB); // Ascending order for time left
      });
    }
    setState(() {}); // Trigger UI update
  }*/

// Example function to calculate remaining time
  int getRemainingTimeValue(Map<String, dynamic> task) {
    if (task["deadline"] != null && task["deadline"].isNotEmpty) {
      try {
        final deadline = DateTime.parse(task["deadline"]); // Parse deadline
        final currentTime = DateTime.now(); // Current time
        final difference =
            deadline.difference(currentTime).inSeconds; // Time left
        return difference > 0 ? difference : 0; // Ensure non-negative values
      } catch (e) {
        print('Error parsing deadline: ${task["deadline"]}'); // Debugging
        return double.maxFinite
            .toInt(); // Invalid deadline pushes task to bottom
      }
    }
    return double.maxFinite.toInt(); // No deadline pushes task to bottom
  }
  ...
  Widget build(BuildContext context) {
    /*return Scaffold(
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      ...
                      log("Latest time: ${DateTime.now().toString()}");
                      toggleDrawer();
                    },
                    child: Container(
                      padding: const EdgeInsets.symmetric(
                  ...
                  GestureDetector(
                    ...
                    child: CircleAvatar(
                      ...
                      backgroundColor: Colors
                          .grey[300], // Background color for the 3D effect
                      child: Container(
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                  ...
                  child: Column(
                    children: [
                      ...
                      /*DropdownButton<String>(
                        ...
                        items: <String>['Date Added', 'Time Left']
                        ...
                        onChanged: (String? newValue) {
                          if (newValue != null) {
                            setState(() {
                              ...
                              sortTasks(); // Sort tasks based on the selected option
                            });
                          }
                        },
                      ),*/
                        ...
                        child: Padding(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              // Sort tasks by addedDate in descending order
                              tasks.sort((a, b) {
                                final dateA = a["addedDate"] != null ? DateTime.parse(a["addedDate"]) : DateTime.now();
                                final dateB = b["addedDate"] != null ? DateTime.parse(b["addedDate"]) : DateTime.now();
      ...
      //             MaterialPageRoute(builder: (context) {
      //               return DeletedTasksPage(deletedTime: DateTime.now());
      //             }),
      //           );
      //         },
      //         icon: Icons.delete,
      ...
      //           end: Alignment.bottomRight,
      //         ),
      //         activeBorder: Border.all(color: Colors.deepPurple, width: 1),
  ...
  .*/
    ...
    return Scaffold(
      body:
          pages[currentIndex], // Display the current screen based on the index
      bottomNavigationBar: FlipBoxNavigationBar(
        currentIndex: currentIndex,
        verticalPadding: 20.0,
        items: <FlipBoxNavigationBarItem>[
          FlipBoxNavigationBarItem(
            ...
            unselectedBackgroundColor: Colors.amber[100]!,
          ),
          FlipBoxNavigationBarItem(
            name: 'Times Up',
            selectedIcon: Icons.timer_off_outlined,
            unselectedIcon: Icons.timer_off,
            selectedBackgroundColor: Colors.red[300]!,
            unselectedBackgroundColor: Colors.red[100]!,
          ),
          FlipBoxNavigationBarItem(
            name: 'Staff',
            selectedIcon: Icons.groups_2_outlined,
            unselectedIcon: Icons.groups_2,
            selectedBackgroundColor: Colors.deepPurple[300]!,
            unselectedBackgroundColor: Colors.deepPurple[100]!,
          ),
        ],
        onTap: (index) {
          setState(() {
            currentIndex = index; // Update the index to switch screens
          });
        },
      ),

      /*floatingActionButton: (_isDrawerOpen && currentIndex == 0)
          ? const SizedBox()
          : SizedBox(
              width: 190,
              height: 50,
